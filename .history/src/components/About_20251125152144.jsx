import React from 'react'

const About = () => {
  return (
    <div id='about'>
        <div className="mb-16 md:px-0 px-5">
             <div className="content">
                <div className="md:col-span-8">
                    <p className="badge">Best Cocktails</p>
                    <h2>
                        Where every detail matters <span>-</span>
                        from muddle to garnish
                    </h2>
                </div>

                <div className="sub-content">
                    <p>Every cocktail we serve is a reflection of our obsession with datail-from the first muddle to the final garnish. That care is what turns a simple drink into something turly memorable.</p>
                </div>

                <div className="sub-content">
                    <p className='md:text-3xl text-xl font-bold'>
                        <span className='text-4xl'>4.5</span>/5
                    </p>
                    <p className="text-sm text-white-100">
                        More than 4,000 customers
                    </p>
                </div>

             </div>
        </div>

        <div className="top-grid">
            <div className="md:col-span-3">
                <div className="noisy">
                    <img src="https://images.unsplash.com/photo-1509042239860-f550ce710b93?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8Y29ja3RhaWx8ZW58MHx8MHx8fDA%3D&w=1000&q=80" alt="cocktail-image" />
                </div>
            </div>
        </div>
    </div>
  )
}

export default About